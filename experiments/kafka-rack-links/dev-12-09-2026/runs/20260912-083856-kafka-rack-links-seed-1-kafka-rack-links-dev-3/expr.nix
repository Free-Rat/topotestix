...
orchestrate {
  ...
  name = "kafka-rack-links-dev-3";
  reportNode = "client1";
  topologyChoices = (builtins.fromJSON "{\".rackaVlans\": 0, \".rackbVlans\": 0, \".rackcVlans\": 0}");
  configChoices = (builtins.fromJSON "{\"client\": {\".environment.etc.topotestix-kafka-cut.text\": 1, \".environment.etc.topotestix-kafka-placement.json.text\": 0}}");
  repetitionToken = "dev-3";
}